...
class InvoiceServices:
    ...
    def _get_shipping_fee(self, country: str, subtotal: float) -> float:
        if country == "TH":
    ...
    def _get_membership_discount(self, membership: str, subtotal: float) -> float:
        if membership == "gold":
    ...
    def compute_total(self, inv: Invoice) -> Tuple[float, List[str]]:
        ...
        shipping = self._get_shipping_fee(inv.country, subtotal)

        discount = self._get_membership_discount(inv.membership, subtotal)
        
        if inv.coupon and inv.coupon.strip() in self._coupon_rate:
            discount += subtotal * self._coupon_rate[inv.coupon.strip()]
        elif inv.coupon and inv.coupon.strip() != "":
            warnings.append("Unknown coupon")

        tax = (subtotal - discount) * self._get_tax_rate(inv.country)
